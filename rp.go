...
var fileName = flag.String("f", "iptables.txt", "-f connip.txt")
...
func (p *Proxy) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if lp, err = r.Cookie("loginParam"); err != nil || lp.Value == "" { // no cookie, then jump to oa
		// 没有登录，去登录服务器校验
		if r.Form.Get("ticket") == "" {
		...
		// set cookie and jump by cookie again
